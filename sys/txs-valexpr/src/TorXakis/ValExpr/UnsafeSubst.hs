...
unsafeSubst ctx  mp = unsafeSubstView . view
  where
    ...
    unsafeSubstView (Vle ve1 ve2)             = unsafeLE (unsafeSubstView (view ve1)) (unsafeSubstView (view ve2))
    unsafeSubstView (Vlt ve1 ve2)             = unsafeLT (unsafeSubstView (view ve1)) (unsafeSubstView (view ve2))
    unsafeSubstView (Vstrinre s r)            = unsafeStrInRe (unsafeSubstView (view s)) r
    unsafeSubstView (Vcstr a c l)             = unsafeCstr ctx a c (map (unsafeSubstView . view) l)
