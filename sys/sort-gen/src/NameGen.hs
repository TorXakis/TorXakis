...
arbitraryReadableName ns = do
    a <- elements adjectives
    f:fs <- elements famousPeople    
    let x:xs = a ++ (toUpper f : fs)
        n = fromNonEmpty $  x :| xs
    if n `Set.member` ns
        then arbitraryReadableNameWithNumber (x:xs) ns
        else return n

arbitraryReadableNameWithNumber :: String -> Set Name -> Gen Name
arbitraryReadableNameWithNumber (x:xs) ns = do
    i <- arbitrary :: Gen Int
    let n = fromNonEmpty $  x :| (xs ++ show i) 
    if n `Set.member` ns
    then arbitraryReadableNameWithNumber (x:xs) ns
    else return n
...
famousPeople = 
    ...
    , "hoover"
      ...
      -- language and is credited with popularizing the term , "debugging" for
