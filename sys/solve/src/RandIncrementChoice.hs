...
randomSolve p ((v,_):xs) i    | vsort v == SortBool =
    do
        b <- lift randomIO
...
randomSolve p ((v,_):xs) i    | vsort v == SortInt =
    do
...
randomSolve p ((v,-123):xs) i    | vsort v == SortString =                 -- abuse depth to encode char versus string
    do
...
randomSolve p ((v,d):xs) i    | vsort v == SortString =
    do
