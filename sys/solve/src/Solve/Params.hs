...
-- randomization
  ...
  [ ( "param_max_rand_depth"     , "4"         , \s ->   not (null s) && all Char.isDigit s
                                                      && ( 0 < (read s::Integer) ) )
  ...
  , ( "param_TrueBins_StringLength"     , "6"         , \s ->   not (null s) && all Char.isDigit s
                                                             && ( 0 < (read s::Integer) )                 )
  ...
  , ( "param_TrueBins_NrOfBins"     , "10"         , \s ->   not (null s) && all Char.isDigit s
                                                          && ( 0 < (read s::Integer) )                      )
            -- param_TrueBins_NrOfBins :: Int (>0)

  , ( "param_IncrementChoice_MaxGeneratedStringLength"     , "10"         , \s ->   not (null s) && all Char.isDigit s
                                                                                 && ( 0 < (read s::Integer) )                      )
            -- param_IncrementChoice_MaxGeneratedStringLength :: Int (>0)

  , ( "param_IncrementChoice_IntRange"     , "65536"         , \s ->   not (null s) && all Char.isDigit s
                                                                    && ( 0 < (read s::Integer) )                      )
            -- param_IncrementChoice_IntRange :: Int (>0) 
            -- default 2^16

  , ( "param_IncrementChoice_IntPower"     , "4"         , \s ->   not (null s) && all Char.isDigit s
                                                                && ( 0 < (read s::Integer) )                      )
            -- param_IncrementChoice_IntPower :: Int (>0)

  , ( "param_RandSolve_IntHalf"  , "1000"      , \s -> not (null s) && all Char.isDigit s                ) 
            -- param_RandSolve_IntHalf :: Int (>0)
            -- half-value for binomial distribution

  , ( "param_RandSolve_IntNum"   , "5"         , \s -> not (null s) && all Char.isDigit s                )
            -- param_RandSolve_IntNum :: Int (>0)
            -- number of positive (negative) partitions

  , ( "param_RandSolve_adtWidth" , "5"         , \s -> not (null s) && all Char.isDigit s               )
